...
public class checkboxGen : MonoBehaviour
{
	...
	public void Start() 
	{
        ...
        for (int i = 0; i < cols; i++)
        {
            ...
            {
                GameObject newBox = Instantiate(column, new Vector3(i * offset, 0, j * offset), Quaternion.identity);
                newBox.transform.SetParent(platform.GetComponent<Transform>());
                newBox.name = i + " " + j;
            }
        }
        //socket stuff
        ...
        socket.On("checkbox", CheckboxTrigger);
    ...
    public void CheckboxTrigger(SocketIOEvent e)
    {
        Debug.Log(e.data);
        string name = e.data.GetField("row") + " " + e.data.GetField("col");
        Debug.Log(name);
        GameObject triggeredBox = GameObject.Find(name);
        triggeredBox.AddComponent<Rigidbody>();
